...
module.exports = {
    config: {
        ...
        longDescription: {
            bn: "অ্যানিমে চরিত্রের ছবি দেখে নাম অনুমান করার খেলা। দৈনিক ১৫ বার খেলার লিমিট, লেডারবোর্ড এবং নতুন অ্যানিমে ক্যারেক্টার এড করার সুবিধা।",
            en: "Guess the anime character name by looking at the picture with daily limit, leaderboard, and add feature.",
            vi: "Đoán tên nhân vật anime bằng cách nhìn vào bức ảnh."
        },
        category: "game",
        guide: {
            bn: '  {pn} | {pn} reset | {pn} info | {pn} top | {pn} add [Character Name] (reply to image)',
            en: '  {pn} | {pn} reset | {pn} info | {pn} top | {pn} add [Character Name] (reply to image)',
            vi: '  {pn} | {pn} reset | {pn} info | {pn} top | {pn} add [Character Name] (reply to image)'
        }
    },

    langs: {
        bn: {
            start: "𝘼 𝙧𝙖𝙣𝙙𝙤𝙢 𝙖𝙣𝙞𝙢𝙚 𝙘𝙝𝙖𝙧𝙖𝙘𝙩𝙚𝙧 𝙝𝙖𝙨 𝙖𝙥𝙥𝙚𝙖𝙧𝙚𝙙! 𝙂𝙪𝙚𝙨𝙨 𝙩𝙝𝙚 𝙣𝙖𝙢𝙚, 𝙗𝙖𝙗𝙮.\n📌 𝙇𝙚𝙛𝙩 𝙩𝙤𝙙𝙖𝙮: %1",
        ...
        try {
            ...
            if (action === "reset") {
                ...
                if (targetUserData && targetUserData.data) {
                    targetUserData.data.aniqz = targetUserData.data.aniqz || {};
                    targetUserData.data.aniqz.count = 0;
                    targetUserData.data.aniqz.todayWins = 0;
                    targetUserData.data.aniqz.todayLosses = 0;
                    targetUserData.data.aniqz.date = today;
            ...
            if (action === "info" || action === "stats") {
                ...
                let targetUserData = await usersData.get(targetID);
                let aniqzInfo = targetUserData?.data?.aniqz || { count: 0, wins: 0, totalPlayed: 0, todayWins: 0, todayLosses: 0, date: today };

                const isTodayData = aniqzInfo.date === today;
                const todayPlayed = isTodayData ? aniqzInfo.count : 0;
                const leftToday = 15 - todayPlayed;
                const todayWins = isTodayData ? (aniqzInfo.todayWins || 0) : 0;
                const todayLosses = isTodayData ? (aniqzInfo.todayLosses || 0) : 0;

                const totalPlayed = aniqzInfo.totalPlayed || 0;
                const totalWins = aniqzInfo.wins || 0;
                const totalLosses = totalPlayed - totalWins;
                const winRate = totalPlayed > 0 ? ((totalWins / totalPlayed) * 100).toFixed(1) : 0;

                const infoMsg = `📊 𝘼𝙉𝙄𝙈𝙀 𝙌𝙐𝙄𝙡 𝙎𝙏𝘼𝙏𝙄𝙎𝙏𝙄𝘾𝙎 𝙄𝙉𝙁𝙊\n` +
                                `───────────────────────\n` +
                                `👤 𝙋𝙡𝙖𝙮𝙚𝙧: ${targetName}\n` +
            ...
            // 3. Top Leaderboard Feature (`animequiz top`)
            if (action === "top" || action === "leaderboard") {
                try {
                    ...
                    for (const u of allUsers) {
                        if (u.data && u.data.aniqz && typeof u.data.aniqz.wins === "number" && u.data.aniqz.wins > 0) {
                            let name = "User";
                            try {
                            ...
                            sortedUsers.push({
                                name,
                                wins: u.data.aniqz.wins,
                                total: u.data.aniqz.totalPlayed || 0
                            });
                        }
                    }
                    ...
                    if (top10.length === 0) {
                        return api.sendMessage("⚠️ | 𝙉𝙤 𝙡𝙚𝙖𝙙𝙚𝙧𝙗𝙤𝙖𝙧𝙙 𝙙𝙖𝙩𝙖 𝙛𝙤𝙪𝙣𝙙 𝙮𝙚𝙩, 𝙗𝙖/𝙗𝙮!", threadID, messageID);
                    }

                    let msg = `🏆 𝘼𝙉𝙄𝙈𝙀 𝙌𝙐𝙄𝙡 𝙏𝙊𝙋 𝟭𝟬 𝙇𝙀𝘼𝘿𝙀𝙍𝘽𝙊𝘼𝙍𝘿 🏆\n───────────────────────\n`;
            ...
            if (!userData.data.aniqz || userData.data.aniqz.date !== today) {
                userData.data.aniqz = { 
                    count: 0, 
                    wins: userData.data.aniqz?.wins || 0, 
                    totalPlayed: userData.data.aniqz?.totalPlayed || 0, 
                    todayWins: 0,
                    todayLosses: 0,
                    date: today 
                };
            ...
            if (userData.data.aniqz.count >= 15) {
                return api.sendMessage("🚫 | 𝙔𝙤𝙪 𝙝𝙖𝙫𝙚 𝙧𝙚𝙖𝙘𝙝𝙚𝙙 𝙩𝙝𝙚 𝙡𝙞𝙢𝙞𝙩 𝙤𝙛 𝙥𝙡𝙖𝙮𝙞𝙣𝙜 𝟭𝟱 𝙖𝙣𝙞𝙢𝙚 𝙦𝙪𝙞𝙯 𝙜𝙖𝙢𝙚𝙨 𝙩𝙤𝙙𝙖𝙮, 𝙗𝙖𝙗𝙮! 𝘾𝙤𝙢𝙚 𝙗𝙖𝙘𝙠 𝙩𝙤𝙢𝙤𝙧𝙧𝙤𝙬.", threadID, messageID);
            }

            userData.data.aniqz.count += 1;
            userData.data.aniqz.totalPlayed = (userData.data.aniqz.totalPlayed || 0) + 1;
            ...
            const remainingLimit = 15 - userData.data.aniqz.count;
    ...
    onReply: async function ({ api, event, Reply, usersData, getLang }) {
        ...
        const { aniqzNames, author, remainingLimit, imgurLink } = Reply;
        const getCoin = 500;
        const getExp = 121;
        ...
        let aniqzDataStore = userData?.data?.aniqz || { count: 0, wins: 0, totalPlayed: 0, todayWins: 0, todayLosses: 0, date: getTodayDate() };

        const today = getTodayDate();
        if (aniqzDataStore.date !== today) {
            aniqzDataStore.count = 0;
            aniqzDataStore.todayWins = 0;
            aniqzDataStore.todayLosses = 0;
            aniqzDataStore.date = today;
        }
        ...
        if (isCorrect) {
            aniqzDataStore.wins = (aniqzDataStore.wins || 0) + 1;
            aniqzDataStore.todayWins = (aniqzDataStore.todayWins || 0) + 1;
            userData.data.aniqz = aniqzDataStore;
            ...
            aniqzDataStore.todayLosses = (aniqzDataStore.todayLosses || 0) + 1;
            userData.data.aniqz = aniqzDataStore;
            await usersData.set(senderID, { data: userData.data });
            ...
            resultMsg = getLang("wrong", aniqzNames.join(", "), remainingLimit);
        }
