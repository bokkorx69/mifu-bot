...
async function generatePinterestCanvas(imageObjects, query, page, totalPages) {
  ...
  ctx.fillText(`Pinterest - Page ${page}/${totalPages}`, canvasWidth / 2, footerY);
...
module.exports = {
  config: {
    ...
    countDown: 10,
    role: 0,
    shortDescription: "Search Pinterest for images with canvas view",
    longDescription: "Search Pinterest for images using API, with canvas view for Browse or direct send.",
    category: "Image",
    guide: {
      en: "{pn} query -[count]\n" +
        "• If count is used, it sends images directly.\n" +
        "• If no count, it shows an interactive canvas.\n" +
        "• Example: {pn} goku -5 (direct send)\n" +
  ...
  ST: async function({ api, args, message, event }) {
    ...
    try {
      ...
      if (!query) {
        return message.reply("Please provide a search query. Example: pinterest goku - 5");
      }

      processingMessage = await message.reply("🔍 Searching on Pinterest...");

      const response = await axios.get(`${await baseApiUrl()}/api/pin/mahmud?query=${encodeURIComponent(query)}&limit=90`);
      ...
      if (count) {
        ...
        return message.reply({
          body: `Here are ${attachments.length} image(s) for "${query}":`,
    ...
    try {
      ...
      if (input === 'next') {
        ...
        if (!isNaN(number) && number > 0) {
          ...
          const imgPath = path.join(cacheDir, `pin_single_${Date.now()}.jpg`);
          await fs.outputFile(imgPath, imgRes.data);
