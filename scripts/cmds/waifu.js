...
module.exports = {
    config: {
        ...
        longDescription: {
            bn: "অ্যানিমে ওয়াইফু দেখে নাম অনুমান করার খেলা। দৈনিক ১৫ বার খেলার লিমিট, লেডারবোর্ড এবং স্ট্যাটস চেক করার সুবিধা।",
            en: "Guess the anime waifu name by looking at the picture with daily limit, leaderboard, and stats feature.",
            vi: "Đoán tên waifu bằng cách nhìn vào bức ảnh với giới hạn hàng ngày, bảng xếp hạng và tính năng thống kê."
        },
        category: "game",
        guide: {
    ...
    langs: {
        bn: {
            start: "✨ | 𝘼 𝙬𝙖𝙞𝙛𝙪 𝙝𝙖𝙨 𝙖𝙥𝙥𝙚𝙖𝙧𝙚𝙙! 𝙂𝙪𝙚𝙨𝙨 𝙝𝙚𝙧 𝙣𝙖𝙢𝙚, 𝙗𝙖𝙗𝙮.\n📌 𝙇𝙚𝙛𝙩 𝙩𝙤𝙙𝙖𝙮: %1",
        ...
        try {
            ...
            if (action === "reset") {
                ...
                if (targetUserData && targetUserData.data) {
                    targetUserData.data.waifu = targetUserData.data.waifu || {};
                    targetUserData.data.waifu.count = 0;
                    targetUserData.data.waifu.todayWins = 0;
                    targetUserData.data.waifu.todayLosses = 0;
                    targetUserData.data.waifu.date = today;
            ...
            if (action === "info" || action === "stats") {
                ...
                let targetUserData = await usersData.get(targetID);
                let gameInfo = targetUserData?.data?.waifu || { count: 0, wins: 0, totalPlayed: 0, todayWins: 0, todayLosses: 0, date: today };
                ...
                const infoMsg = `📊 𝙒𝘼𝙄𝙁𝙐 𝙂𝘼𝙈𝙀 𝙎𝙏𝘼𝙏𝙄𝙎𝙏𝙄𝘾𝙎 𝙄𝙉𝙁𝙊\n` +
                                `───────────────────────\n` +
                                `👤 𝙋𝙡𝙖𝙮𝙚𝙧: ${targetName}\n` +
            ...
            if (action === "top" || action === "leaderboard") {
                try {
                    ...
                    for (const u of allUsers) {
                        if (u.data && u.data.waifu && typeof u.data.waifu.wins === "number" && u.data.waifu.wins > 0) {
                            let name = "User";
                            try {
                            ...
                            sortedUsers.push({
                                name,
                                wins: u.data.waifu.wins,
                                total: u.data.waifu.totalPlayed || 0
                            });
                        }
                    }
                    ...
                    let msg = `🏆 𝙒𝘼𝙄𝙁𝙐 𝙏𝙊𝙋 𝟭𝟬 𝙇𝙀𝘼𝘿𝙀𝙍𝘽𝙊𝘼𝙍𝘿 🏆\n───────────────────────\n`;
            ...
            if (!userData.data.waifu || userData.data.waifu.date !== today) {
                userData.data.waifu = { 
                    count: 0, 
                    wins: userData.data.waifu?.wins || 0, 
                    totalPlayed: userData.data.waifu?.totalPlayed || 0, 
                    todayWins: 0,
                    todayLosses: 0,
                    date: today 
                };
            ...
            if (userData.data.waifu.count >= 15) {
                return api.sendMessage("🚫 | 𝙔𝙤𝙪 𝙝𝙖𝙫𝙚 𝙧𝙚𝙖𝙘𝙝𝙚𝙙 𝙩𝙝𝙚 𝙡𝙞𝙢𝙞𝙩 𝙤𝙛 𝙥𝙡𝙖𝙮𝙞𝙣𝙜 𝟭𝟓 𝙬𝙖𝙞𝙛𝙪 𝙜𝙖𝙢𝙚𝙨 𝙩𝙤𝙙𝙖𝙮, 𝙗𝙖𝙗𝙮! 𝘾𝙤𝙢𝙚 𝙗𝙖𝙘𝙠 𝙩𝙤𝙢𝙤𝙧𝙧𝙤𝙬.", threadID, messageID);
            }

            userData.data.waifu.count += 1;
            userData.data.waifu.totalPlayed = (userData.data.waifu.totalPlayed || 0) + 1;
            ...
            const remainingLimit = 15 - userData.data.waifu.count;
            ...
            threadID,
            (err, info) => {
                if (err) return api.sendMessage("❌ | 𝙁𝙖𝙞𝙡𝙚𝙙 𝙩𝙤 𝙨𝙚𝙣𝙙 𝙬𝙖𝙞𝙛𝙪 𝙞𝙢𝙖𝙜𝙚, 𝙗𝙖𝙗𝙮!", threadID);
    ...
    onReply: async function ({ api, event, Reply, usersData, getLang }) {
        ...
        const { waifuNames, author, remainingLimit, imgurLink } = Reply;
        const getCoin = 500;
        const getExp = 121;
        ...
        let gameDataStore = userData?.data?.waifu || { count: 0, wins: 0, totalPlayed: 0, todayWins: 0, todayLosses: 0, date: getTodayDate() };
        ...
        const isCorrect = waifuNames.some(name => name.toLowerCase() === reply);
        ...
        if (isCorrect) {
            ...
            userData.data.waifu = gameDataStore;
            ...
            resultMsg = getLang("wrong", waifuNames[0], remainingLimit);
        }
