...
module.exports = {
  config: {
    ...
    role: 0,
    category: "game",
    guide: "{pn} | {pn} en | {pn} reset | {pn} info",
  },
  ...
  onStart: async function ({ api, event, usersData, args }) {
    ...
    if (action === "reset") {
      ...
      if (!adminIDs.includes(userId)) {
        return api.sendMessage("❌ | 𝗢𝐧𝐥𝐲 𝐨𝐰𝐧𝐞𝐫 𝐜𝐚𝐧 𝐫𝐞𝐬𝐞𝐭 𝐭𝐡𝐞 𝐪𝐮𝐢𝐳 𝐥𝐢𝐦𝐢𝐭, 𝐛𝐚𝐛𝐲!", event.threadID, event.messageID);
      }
      ...
      if (targetUserData && targetUserData.data) {
        targetUserData.data.quiz = targetUserData.data.quiz || {};
        targetUserData.data.quiz.count = 0;
        targetUserData.data.quiz.date = today;
        await usersData.set(targetID, { data: targetUserData.data });

        return api.sendMessage(`✅ | 𝐐𝐮𝐢𝐳 𝐥𝐢𝐦𝐢𝐭 𝐡𝐚𝐬 𝐛𝐞𝐞𝐧 𝐫𝐞𝐬𝐞𝐭 𝐭𝐨 𝟎 𝐟𝐨𝐫 **${targetName}** ✨`, event.threadID, event.messageID);
      } else {
    ...
    if (action === "info" || action === "stats") {
      ...
      let targetUserData = await usersData.get(targetID);
      let quizInfo = targetUserData?.data?.quiz || { count: 0, wins: 0, totalPlayed: 0, date: today };
      ...
      const infoMsg = `📊 𝐐𝐔𝐈𝐙 𝐒𝐓𝐀𝐓𝐈𝐒𝐓𝐈𝐂𝐒 𝐈𝐍𝐅𝐎\n` +
                      `───────────────────────\n` +
                      `👤 𝐏𝐥𝐚𝐲𝐞𝐫: ${targetName}\n` +
    ...
    let quizDataStore = currentUserData?.data?.quiz || { count: 0, wins: 0, totalPlayed: 0, date: "" };
    ...
    if (quizDataStore.count >= 15) {
      return api.sendMessage("🚫 | 𝐘𝐨𝐮 𝐡𝐚𝐯𝐞 𝐫𝐞𝐚𝐜𝐡𝐞𝐝 𝐭𝐡𝐞 𝐥𝐢𝐦𝐢𝐭 𝐨𝐟 𝐩𝐥𝐚𝐲𝐢𝐧𝐠 𝟏𝟓 𝐪𝐮𝐢𝐳𝐳𝐞𝐬 𝐭𝐨𝐝𝐚𝐲, 𝐛𝐚𝐛𝐲! 𝐂𝐨𝐦𝐞 𝐛𝐚𝐜𝐤 𝐭𝐨𝐦𝐨𝐫𝐫𝐨𝐰.", event.threadID, event.messageID);
    }
    ...
    currentUserData.data.quiz = quizDataStore;
    ...
    try {
      ...
      if (!quizData) {
        const response = await axios.get("https://azadx69x.is-a.dev/api/quiz");
      ...
      if (!quizData) {
        throw new Error("Failed to fetch quiz data from available APIs.");
      }
      ...
      api.sendMessage(quizMsg, event.threadID, (error, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          ...
          remainingLimit: 15 - quizDataStore.count
        });
      }, event.messageID);
  ...
  onReply: async ({ event, api, Reply, usersData }) => {
    ...
    let quizDataStore = userData?.data?.quiz || { count: 0, wins: 0, totalPlayed: 0, date: getTodayDate() };
    ...
    if (userReply === correctAnswer || event.body.toLowerCase().trim() === correctAnswer) {
      ...
      await usersData.set(author, {
        ...
        data: { ...userData.data, quiz: quizDataStore },
      });
